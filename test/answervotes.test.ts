...
describe("Test Answer Votes CRUD", () => {
  let res: Response;
  ...
  afterEach(async () => {
    ...
    await prisma.$executeRaw('TRUNCATE "AnswerVote" CASCADE;');
  ...
  test("Test Vote CREATE.", async () => {
    ...
    res = await request
      ...
      .send({ answerId: answer.id, type: "UPVOTE" });
    ...
    // * Try to vote on the same answer
    res = await request
      .post("/answer/votes")
      .set("Authorization", `Bearer ${token}`)
      .send({ answerId: answer.id, type: "UPVOTE" });
  ...
  test("Test Vote UPDATE.", async () => {
    ...
    // checking
    const updated = await prisma.answerVote.findFirst();
    expect(updated.type).toEqual("DOWNVOTE");

    // * Try to update the answer id
    res = await request
      .patch(`/answer/votes/${vote.id}`)
      .set("Authorization", `Bearer ${token}`)
      .send({ answerId: answer.id, type: "DOWNVOTE" });
  ...
  test("Test Vote DESTROY.", async () => {
    ...
    expect(await prisma.answerVote.count()).toBe(0);

    // * Delete a vote that doesn't exists
    res = await request
      .delete(`/answer/votes/${uuid()}`)
      .set("Authorization", `Bearer ${token}`);
    expect(res.status).toBe(404);
  });
});
